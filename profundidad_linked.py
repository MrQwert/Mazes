"""
El presente código resuelve laberintos utilizando el algoritmo de búsqueda en profundidad (Depth-First Search o DFS).
"""
# Se importan las librerías necesarias
from time import sleep
from os import system

SPEED = 150  # Se establece la velocidad a la que se imprimirá el proceso de búsqueda.
...
# Se define la clase 'Profunidad' para implementar el algoritmo de búsqueda en profundidad
class Profundidad:
    """
    ...
    def comprobar_estado(self):
        ...
        """
        try:
            estado = self.abierto.pop(-1)  # Se intenta obtener el siguiente estado de la lista de estados abiertos (LIFO)
        ...
        if self.estado_actual[0] == self.estado_final[0]:  # Se comprueba si el estado actual es igual al estado final
            return True
        if self.estado_actual[0] != self.estado_inicial[
            0]:  # Si el estado actual no es el estado final ni el estado inicial, se actualiza el laberinto con el símbolo '#' para marcar la posición actual
            self.laberinto[estado[0][1]] = self.laberinto[estado[0][1]][:estado[0][0]] + '#' + self.laberinto[
                                                                                                   estado[0][1]][
                                                                                               estado[0][0] + 1:]
        return False

    def imprime_laberinto(self):  # Función para imprimir el laberinto resuelto por el algoritmo
...
sol = Profundidad(laberinto)  # Se instancia la clase Profundidad indicando el laberinto
...
while state != sol.estado_inicial[
    0]:  # Se imprime la solución final paso a paso, marcando los movimientos realizados con el símbolo '@'
